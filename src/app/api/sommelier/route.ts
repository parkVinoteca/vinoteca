...
import { displayJapaneseWine, japaneseDisplayInstruction, readJapaneseWine } from '@/lib/server/wineDisplay'
...
export async function POST(req: Request) {
  try {
    ...
    const parsed = parseResult(blocks.filter((b: { type: string }) => b.type === 'text').map((b: { text: string }) => b.text).join(''))
    const result = validateSommelier(parsed)
    ...
    const displayed = displayJapaneseWine(result, result, readJapaneseWine(parsed), image.lang)
    return Response.json({ result: { ...displayed, sources: sources.slice(0, 10), researchedAt: new Date().toISOString() } }, { headers: { 'Cache-Control': 'no-store' } })
