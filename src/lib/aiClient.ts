...
export async function analyzeImage(endpoint: 'label' | 'sommelier', image: { base64: string; mediaType: string }, lang: Language, signal?: AbortSignal) {
  return analyzeRequest(endpoint, { imageBase64: image.base64, imageMediaType: image.mediaType }, lang, signal)
}
export async function analyzeSommelier(hints: {wineName:string;producer:string;vintage:string}, lang:Language, signal?:AbortSignal) {
  return analyzeRequest('sommelier',hints,lang,signal)
}
async function analyzeRequest(endpoint:'label'|'sommelier',payload:object,lang:Language,signal?:AbortSignal) {
  const { data: { session }, error } = await supabase.auth.getSession()
  if (error || !session) throw new Error(messages[lang].sign_in_required)
  try {
    const response = await fetch(`/api/${endpoint}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
      body: JSON.stringify({ ...payload, lang }), signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(endpoint === 'label' ? 175000 : 85000)]) : AbortSignal.timeout(endpoint === 'label' ? 175000 : 85000),
    })
