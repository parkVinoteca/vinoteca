...
import { japaneseDisplayInstruction, readJapaneseWine, type JapaneseWine } from '@/lib/server/wineDisplay'

type Label = ReturnType<typeof validateLabel>
export type GrapeResearch = { status: 'label' | 'verified' | 'not_found' | 'unavailable' | 'knowledge'; source: string | null; vintageMatched: boolean; blendRatio: string | null }
const empty = (status: GrapeResearch['status']): GrapeResearch => ({ status, source: null, vintageMatched: false, blendRatio: null })

// Ignore typography differences, not missing source words or unsupported facts.
export const normalizeEvidence = (value:string) => value.normalize('NFKC').toLowerCase().replace(/[’‘]/g,"'").replace(/[‐‑–—]/g,'-').replace(/\s+/g,' ').trim()
...
export async function enrichGrapes(label: Label, lang: 'ja' | 'ko', key?: string, researchCritics = false, reading?: WineReading) {
  ...
  let base: Label & { wineResearch?: WineResearch; japanese?: JapaneseWine } = reading ? unverifiedWine(reading) : label
  ...
  try {
    ...
    const firstDocument = typeof result.source === 'string' ? documents.get(result.source) : null
    const evidencePresent = firstDocument && Array.isArray(result.grapes) && result.grapes.length > 0 &&
      result.grapes.every((g: {name?:unknown;evidence?:unknown}) => typeof g?.name === 'string' && typeof g.evidence === 'string' &&
        normalizeEvidence(firstDocument).includes(normalizeEvidence(g.evidence)) && normalizeEvidence(g.evidence).includes(normalizeEvidence(g.name)))
    // Reuse a complete grounded answer. Do not pay to re-extract facts already present.
    if (!(reading && firstDocument && verifyWineIdentity(result, firstDocument, reading) && evidencePresent)) {
    ...
    if (reading) {
      ...
      base = { ...identity, japanese: readJapaneseWine(result), wineResearch: { status: 'verified', source: result.source as string, catalogId: null } }
    }
