...
export async function enrichGrapes(label: Label, lang: 'ja' | 'ko', key?: string) {
  ...
  try {
    ...
    if (!documents.size) return { ...label, grapeResearch: empty('unavailable') }
    ...
    if (result.wineMatched !== true || !document || !Array.isArray(result.grapes) || !result.grapes.length || result.grapes.length > 12) return { ...label, grapeResearch: empty('not_found') }
    ...
    if (grapeVariety.length > 300) return { ...label, grapeResearch: empty('not_found') }
    ...
    return { ...label, grapeVariety, grapeResearch: { status: 'verified', source: result.source, vintageMatched, blendRatio } as GrapeResearch }
