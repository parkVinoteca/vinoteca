    ...
    return (
      <div className="max-w-lg mx-auto">
        ...
        <div className="p-4">
          ...
          <CriticScores value={selected.critic_scores} lang={lang} />
          ...
          {/* My Score */}
          {personalRating(selected) !== null && (
            <div className="bg-gradient-to-b from-gold-500 to-gold-600 text-white p-4 text-center mb-3">
              <div className="text-xs text-white mb-1">{lang === 'ja' ? 'マイスコア' : '내 점수'}</div>
              <div className="font-serif text-5xl font-bold">{formatRating(selected)}<span className="text-base"> / 5</span></div>
              {!selected.stars && selected.score && <p className="text-xs mt-2">{t.tastingGuide.legacy}</p>}
            </div>
          )}

          {/* Tasting Data */}
          {[
            { label: lang === 'ja' ? '外観' : '외관', data: [describe(t.tasting.colorHue,selected.color_hue), describe(t.tasting.colorDepth,selected.color_depth), describe(t.tasting.clarity,selected.clarity), describe(t.tasting.viscosity,selected.viscosity)].filter(Boolean) },
            { label: lang === 'ja' ? '香り' : '향', data: [describe(t.tasting.noseIntensity,selected.nose_intensity), selected.nose_condition, describe(t.tastingGuide.development,selected.nose_development), ...(selected.aromas || [])].filter(Boolean) },
            { label: lang === 'ja' ? '味わい' : '미각', data: [describe(t.tasting.sweetness,selected.sweetness), describe(t.tasting.acidity,selected.acidity), describe(t.tasting.tannin,selected.tannin), selected.tannin_texture, selected.mousse, describe(t.tasting.alcohol,selected.alcohol), describe(t.tasting.body,selected.body), describe(t.tasting.flavorIntensity,selected.flavor_intensity), describe(t.tasting.finish,selected.finish)].filter(Boolean) },
          ].map(section => section.data.length > 0 && (
            <div key={section.label} className="card p-3 mb-3">
              <div className="text-xs font-medium text-gold-700 mb-2">{section.label}</div>
              <div className="flex flex-wrap gap-1">
                {section.data.map(d => <span key={d} className="text-xs bg-gold-900/20 text-gold-700 px-2 py-0.5">{d}</span>)}
              </div>
            </div>
          ))}
            ...
            <div className="bg-gold-900/20 border border-gold-900/30 p-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                {[
                  ...
                  <div key={k}>
                    ...
                    <div className="font-medium text-gold-700">{v}</div>
                  </div>
                ))}
              </div>
              {(selected.answer_producer || selected.answer_wine) && (
                <div className="mt-3 pt-3 border-t border-gold-900/30">
                  ...
                  <div className="font-medium text-gold-700">{selected.answer_producer} {selected.answer_wine}</div>
                </div>
              )}
            </div>
          )}
          ...
          {filtered.map(tasting => (
            <button key={tasting.id} onClick={() => setSelected(tasting)} className="w-full card p-3 flex items-center gap-3 hover:bg-gold-900/20 transition-colors text-left">
              ...
              {personalRating(tasting) !== null && (
                <div className="font-serif text-2xl font-bold text-gold-700 flex-shrink-0">{formatRating(tasting)}<span className="text-xs"> / 5</span></div>
              )}
            </button>
          ))}
        </div>
