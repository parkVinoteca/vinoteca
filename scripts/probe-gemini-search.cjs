...
if (process.env.VERCEL_ENV !== 'preview' || !process.env.VERCEL_GIT_COMMIT_MESSAGE?.includes('[probe-gemini-catalog-20260921]')) process.exit(0)
...
async function main() {
  ...
  if (process.env.GEMINI_API_KEY) {
    ...
    try {
      const models = await providerFetch('https://generativelanguage.googleapis.com/v1beta/models?pageSize=1000', {headers:{'x-goog-api-key':process.env.GEMINI_API_KEY}}, 15000)
      result.availableModels = (models.models||[]).filter(m=>m.supportedGenerationMethods?.includes('generateContent')).map(m=>m.name)
      if (!result.availableModels.includes('models/gemini-2.5-flash-lite')) {
        result.status='free_search_model_not_listed'
