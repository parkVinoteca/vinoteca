 ...
 for(const wine of wines){
  ...
  for(const endpoint of ['label','sommelier']){
   ...
   const providerFetch=async(url,init,timeout)=>{
    ...
    try{const data=await real.providerFetch(url,init,timeout);calls.push({model,seconds:(Date.now()-t)/1000,status:'ok',usage:data.usage||data.usageMetadata||null,stop:data.stop_reason||data.candidates?.[0]?.finishReason});return data}
