...
운영 DB는 아직 기존 한도(소믈리에 20/일·100/월)를 사용한다. `20260918_product_foundation.sql` 적용 후 일본 시간 기준 무료 회원은 테이스팅 월10회·소믈리에 월5회, 유료 회원은 테이스팅 무제한·소믈리에 월50회다. 결제 연동 전에는 모두 무료로 생성한다. 값은 `subscription_limits`에서 조정한다. 전체 프로젝트 월 소믈리에 500·라벨 2000, 동일 기능 호출 간 10초 제한은 유지한다. 실패한 외부 호출도 예약에 포함한다.
...
`supabase/migrations/20260918_product_foundation.sql`은 로컬 PGlite 검증만 완료했으며 운영 DB에는 아직 적용하지 않았다. 앱 배포 전에 먼저 적용해야 한다.
